...
_Side note, but an important one: none of this applies to "long-form" content (or replies, or anything) that either **has** a title or **wasn't** posted through Micropub! That is, **actual** WordPress posts (Articles, in IndieWeb jargon) that happen to contain, e.g., a backlink with a `u-bookmark-of` class aren't at all affected by this plugin in the first place._
...
Your theme will have to make sure they're correctly "hidden" on the front end, though! (Note titles accidently given a `p-name` class will cause Microformats parsers to treat Notes like Articles. Something similar goes for Notes in RSS.)

The most common RSS feeds, on the other hand, are taken care of by this plugin: Note titles are automatically scrubbed from them. (That is, if you're not already using a modified RSS template.)

That said, Bookmarks are a common exception and are often given the title of the _actually bookmarked page_, and that's why this plugin will _not_ treat them like other "Notes."

Use `add_filter( 'iwcpt_ignore_bookmark_titles', '__return_false' );` to reverse this behavior and have this plugin autogenerate bookmark titles, too.
